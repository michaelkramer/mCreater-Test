	...
	public static class WorldVariables extends WorldSavedData {
		...
		public void syncData(World world) {
			...
			if (world.isRemote) {
				...
				Test.PACKET_HANDLER.send(PacketDistributor.DIMENSION.with(world.dimension::getType), new WorldSavedDataSyncMessage(1, this));
			}
		}
	...
	public static class MapVariables extends WorldSavedData {
		...
		public void syncData(World world) {
			...
			if (world.isRemote) {
				...
				Test.PACKET_HANDLER.send(PacketDistributor.ALL.noArg(), new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	public static class WorldSavedDataSyncMessage {
		...
		private static void syncData(WorldSavedDataSyncMessage message, LogicalSide side, World world) {
			if (side.isServer()) {
				...
				if (message.type == 0) {
					Test.PACKET_HANDLER.send(PacketDistributor.ALL.noArg(), message);
					world.getServer().getWorld(DimensionType.OVERWORLD).getSavedData().set(message.data);
				} else {
					Test.PACKET_HANDLER.send(PacketDistributor.DIMENSION.with(world.dimension::getType), message);
